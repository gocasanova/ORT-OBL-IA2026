...
# Grid deliberadamente chico: los únicos ejes son discretización y parámetros
# estándar de Q-Learning.
...
GAMMA_VALUES = [0.99]
EPSILON_VALUES = [1.0]
EPSILON_MIN_VALUES = [0.05]
EPSILON_DECAY_VALUES = [0.995]

MANUAL_CONFIGS = [
    {
        "config_name": "baseline_40x40_a11",
        "position_bins": 40,
        "velocity_bins": 40,
        "action_bins": 11,
        "alpha": 0.1,
...
def build_grid_configs() -> list[dict[str, Any]]:
    ...
    for (
        ...
        name = (
            ...
            f"_alpha{_label(alpha)}_gamma{_label(gamma)}"
            f"_eps{_label(epsilon)}_min{_label(epsilon_min)}"
            f"_decay{_label(decay)}"
        )
        configs.append(
            {
                "config_name": name,
